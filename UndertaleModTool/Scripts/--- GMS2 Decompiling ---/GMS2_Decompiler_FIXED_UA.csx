...
public class NamePathAndOrder
{
	public string name { get; set; }
	public string path { get; set; }
	public int order { get; set; }
}
public class GMResourceOrderFile // not really needed tbh
{
	public List<NamePathAndOrder> ResourceOrderSettings { get; set; }
	public List<NamePathAndOrder> FolderOrderSettings { get; set; }
}
		...
		for (var j = 0; j < sprite.Textures.Count; j++)
		{
			...
			if (frame.Texture != null)
			{
				...
				try
				{
					...
					{
						if (skipErrors)
							errorList.Add($"{exportedSprite.name} - SWF or SPINE sprite skipped");
				...
				foreach (var match in varDefs)
				{
					...
					if (find.Length >= 2)
					{
						...
						if (varValue.EndsWith(";"))
							varValue = varValue.Remove(varValue.Length - 1, 1);
...
void DumpRoom(UndertaleRoom room)
{
	...
	exportedRoom.roomSettings.persistent = room.Persistent;
	...
	// room creation code
	if (room.CreationCodeId != null && !COMP)
	{
		string code = "";
			...
			if (l.Data != null)
			{
				...
				switch (l.LayerType)
				{
					case UndertaleRoom.LayerType.Instances: // Instances
						...
						if ((lid == 0 && l.LayerDepth == 0) || (lid != 0 && l.LayerDepth == room.Layers[lid - 1].LayerDepth + 100) || (l != room.Layers.Last() && l.LayerDepth == room.Layers[lid + 1].LayerDepth - 100))
							instLayer.userdefinedDepth = true;
									...
									try
									{
										...
										foreach (var match in varDefs)
										{
											...
											if (find.Length >= 2)
											{
												...
												// try to pinpoint which parent object has the variable
												var pinpoint = i.ObjectDefinition;
												string pinpointCode = "";
												bool pinpointFound = false;

												while (pinpoint.ParentId != null)
												{
													if (pinpoint.Events[14].Count == 0)
													{
														pinpoint = pinpoint.ParentId;
														continue;
													}
													foreach (var j in pinpoint.Events[14])
													{
														pinpointCode = decompileCode(j.Actions[0].CodeId);
														if (!pinpointCode.Contains("\n" + varName + " = "))
															pinpoint = pinpoint.ParentId;
														else
														{
															pinpointFound = true;
												...
												inst.properties.Add(new GMOverriddenProperty
												{
													...
													{
														name = varName,
														path = $"objects/{pinpoint.Name.Content}/{pinpoint.Name.Content}.yy"
													},
													objectId = new IdReference
													{
														name = pinpoint.Name.Content,
														path = $"objects/{pinpoint.Name.Content}/{pinpoint.Name.Content}.yy"
													},
													value = varValue
												});
					...
					case UndertaleRoom.LayerType.Tiles: // Tiles
						...
						if ((lid == 0 && l.LayerDepth == 0) || (lid != 0 && l.LayerDepth == room.Layers[lid - 1].LayerDepth + 100) || (l != room.Layers.Last() && l.LayerDepth == room.Layers[lid + 1].LayerDepth - 100))
							tileLayer.userdefinedDepth = true;
					...
					case UndertaleRoom.LayerType.Background: // Backgrounds
						...
						if (bg != null)
						{
							if (bg.Sprite != null)
							{
								...
								if (bg.AnimationSpeed != bg.Sprite.GMS2PlaybackSpeed
								|| (int)bg.AnimationSpeedType != (int)bg.Sprite.GMS2PlaybackSpeedType)
									bgLayer.userdefinedAnimFPS = true;
							}
					...
					case UndertaleRoom.LayerType.Assets: // Assets
						...
						if ((lid == 0 && l.LayerDepth == 0) || (lid != 0 && l.LayerDepth == room.Layers[lid - 1].LayerDepth + 100) || (l != room.Layers.Last() && l.LayerDepth == room.Layers[lid + 1].LayerDepth - 100))
							assetLayer.userdefinedDepth = true;
					...
					case UndertaleRoom.LayerType.Effect: // Effects
						...
						if ((lid == 0 && l.LayerDepth == 0) || (lid != 0 && l.LayerDepth == room.Layers[lid - 1].LayerDepth + 100) || (l != room.Layers.Last() && l.LayerDepth == room.Layers[lid + 1].LayerDepth - 100))
							effectLayer.userdefinedDepth = true;
...
void DumpTileset(UndertaleBackground bg)
{
	...
	// in order to get both working sprites and null sprites working, I have to set most settings to 0
	// as leaving them in here causes issues
	// and the try + catch has to apply correct settings to working sprites
	var exportedSprite = new GMSprite
	{
		name = bg.Name.Content + (Data.IsGameMaker2() ? "_sprite" : ""),
		width = 0,
		height = 0,
		bbox_left = 0,
		bbox_right = 0,
	...
	try {
	// if sprite has image, set to correct settings
		exportedSprite.width = (int)bg.Texture.TargetWidth;
		exportedSprite.height = (int)bg.Texture.TargetHeight;
		exportedSprite.bbox_right = (int)bg.Texture.TargetWidth - 1;
		exportedSprite.bbox_bottom  = (int)bg.Texture.TargetHeight - 1;
	}
	// if null sprite, do nothing, but log it
	catch (Exception e) {
	// Log Null Sprite
		errorList.Add($"{exportedSprite.name} - Null Sprite: No associated Image found");
	}

	// in case that sprite name already exists
	var overlap = exportedSprite.name;
	var overlapCount = 1;
	while (Data.Sprites.ByName(overlap) != null)
		overlap = exportedSprite.name + (++overlapCount);
	exportedSprite.name = overlap;
...
void DumpScript(UndertaleScript scr)
{
	...
	{
		// not 2.3 so just do a single function
		...
		code = $"function {exportedScript.name}()\n{{\n{code}}}\n";
	}

	// add function() and add tab to every line
	if (code == "" && Data.IsVersionAtLeast(2, 3))
	{
		// if yyc AND 2.3, write subfunctions
		var i = Data.Scripts.IndexOf(scr) + 1;
		var n = Data.Scripts[i].Name.Content;

		while (i < Data.Scripts.Count && n.StartsWith("gml_Script_"))
		{
			if (n.Contains("_gml_RoomCC_") || n.Contains("_gml_Object_"))
				break;
			code += $"function {n.Replace("gml_Script_", "")}()\n{{\n	\n}}\n\n";

			if (++i >= Data.Scripts.Count)
				break;
			n = Data.Scripts[i].Name.Content;
		}
	}
	if (code != "")
...
async Task DumpScripts()
{
	...
	if (CSTM.Count == 0)
	{
		...
		if (code != "")
		{
			...
			// finish
			File.WriteAllText(rootPath + scriptPath + globalinit.name + ".gml", "// Script generated by loypoll's decompiler\n" + code);
...
#endregion
#region GMS1 Asset Dumpers

public const string GMS1header = "<!--This Document is generated by GameMaker, if you edit it by hand then you do so at your own risk!-->";
XmlWriter doXml(string path)
{
	...
	writer.WriteRaw(GMS1header + "\n");
	return writer;
}
void WriteElementValue<T>(XmlWriter writer, string element, T value)
{
	writer.WriteStartElement(element);
	if (value is bool)
		writer.WriteValue((bool)(object)value ? -1 : 0);
	else
...
// sprites
void DumpSpriteGMS1(UndertaleSprite sprite)
{
	Directory.CreateDirectory(rootPath + "sprites");
	Directory.CreateDirectory(rootPath + "sprites\\images");

	using (var writer = doXml(rootPath + $"sprites\\{sprite.Name.Content}.sprite.gmx"))
	{
		writer.WriteStartElement("sprite"); // <sprite>

		WriteElementValue(writer, "type", 0);
		WriteElementValue(writer, "xorig", sprite.OriginX);
		WriteElementValue(writer, "yorigin", sprite.OriginY);

		var colkind = 0;
		switch((uint)sprite.SepMasks)
		{
			case 0: colkind = 1; break;
			case 1: colkind = 0; break;
			case 2: colkind = 3; break;

			// no way to properly figure these out
		}
		WriteElementValue(writer, "colkind", colkind); // Precise, Rectangle, Ellipse, Diamond
		WriteElementValue(writer, "coltolerance", 0); // *
		WriteElementValue(writer, "sepmasks", (sprite.CollisionMasks.Count > 1));
		WriteElementValue(writer, "bboxmode", sprite.BBoxMode); // Automatic, Full image, Manual
		WriteElementValue(writer, "bbox_left", sprite.MarginLeft);
		WriteElementValue(writer, "bbox_right", sprite.MarginRight);
		WriteElementValue(writer, "bbox_top", sprite.MarginTop);
		WriteElementValue(writer, "bbox_bottom", sprite.MarginBottom);
		WriteElementValue(writer, "HTile", false); // *
		WriteElementValue(writer, "VTile", false); // *

		int tgrp = 0;
		// TODO

		writer.WriteStartElement("TextureGroups");
		WriteElementValue(writer, "TextureGroup0", tgrp);
		writer.WriteEndElement();

		WriteElementValue(writer, "For3D", false); // *
		WriteElementValue(writer, "width", sprite.Width);
		WriteElementValue(writer, "height", sprite.Height);

		writer.WriteStartElement("frames");
		for(var i = 0; i < sprite.Textures.Count; i++)
		{
			var frame = sprite.Textures[i];

			writer.WriteStartElement("frame");
			writer.WriteAttributeString("index", $"{i}");
			writer.WriteString(@$"images\{sprite.Name.Content}_{i}.png");
			writer.WriteEndElement();
			...
			if (frame.Texture != null)
			{
				...
				try
				{
					...
					{
						if (skipErrors)
							errorList.Add($"{sprite.Name.Content} - Non-zero type sprite skipped");
						throw new Exception(); // to get out of the try catch
					}
...
string decompileCode(UndertaleCode codeId)
{
	...
	// decompile OR read off a .gml file
	string code;
	...
	// fix variables based on their name. (might fuck up at times)
	...
	// fix dogscepter code.
	assetRegex<UndertaleGameObject>(new Regex(@"instance_create\(.+, (\d+)\)"), Data.GameObjects);
	...
	if (Data.IsVersionAtLeast(2, 3))
	{
		quickRegex(new Regex(@"((.+\.)\2)")); // obj.obj.var turns into obj.var
		code = code.Replace("gml_Script_", ""); // quickRegex(new Regex(@"[^ ]*(gml_Script_(.+\((?:.*)\)))"));
		quickRegex(new Regex(@"function.*?(?=(\n    return;)())", RegexOptions.Singleline)); // removes useless return; at the end of a function
	}

	// color constants
	code = code.Replace("16777215", "c_white");

	// end code with newline
	if (!code.EndsWith("\n"))
		code += "\n";
	return code;
}
...
_CMBT.Click += (o, e) =>
{
	...
	form2.Controls.Add(includeCheck);
	*/
...
tooltip.SetToolTip(_EROR, "If unchecked, all errors will be\nlogged in a text file.");
...
tooltip.SetToolTip(_APND, "Adds the decompiled resources\nto an existing project file.");

var _TWEK = new CheckBox()
{
...
FORM.Controls.Add(_TWEK);
tooltip.SetToolTip(_TWEK, "Tweaks the decompiled code using regex\nto attempt to fix SOME asset names.");
...
OKBT.Click += (o, s) =>
{
	...
	else if (!GMS1 && !Data.IsGameMaker2() && (_SCPT.Checked || _OBJT.Checked || _ROOM.Checked))
	{
	...
	else if (GMS1 && Data.IsGameMaker2())
	{
...
if (!GMS1)
{
	...
	{
		SetupProgress("Exporting Extensions...", 0, Data.Extensions.Count);
		await DumpExtensions();
	}

	if (SCPT || CSTM.Count > 0)
	{
		SetupProgress("Exporting Scripts...", 0, Data.Scripts.Count);
		await DumpScripts();
	}

	if (TMLN || CSTM.Count > 0)
	{
		SetupProgress("Exporting Timelines...", 0, Data.Timelines.Count);
		await DumpTimelines();
	}

	if (SOND || CSTM.Count > 0)
	{
		SetupProgress("Exporting Sounds...", 0, Data.Sounds.Count);
		await DumpSounds();
	}

	if (SHDR || CSTM.Count > 0)
	{
		SetupProgress("Exporting Shaders...", 0, Data.Shaders.Count);
		await DumpShaders();
	}

	if (PATH || CSTM.Count > 0)
	{
		SetupProgress("Exporting Paths...", 0, Data.Paths.Count);
		await DumpPaths();
	}

	if ((ACRV || CSTM.Count > 0) && Data.AnimationCurves != null)
	{
		SetupProgress("Exporting Animation Curves...", 0, Data.AnimationCurves.Count);
		await DumpCurves();
	}

	if ((SEQN || CSTM.Count > 0) && Data.Sequences != null)
	{
		SetupProgress("Exporting Sequences...", 0, Data.Sequences.Count);
		await DumpSequences();
	}

	if (FONT || CSTM.Count > 0)
	{
		SetupProgress("Exporting Fonts...", 0, Data.Fonts.Count);
		await DumpFonts();
	}
