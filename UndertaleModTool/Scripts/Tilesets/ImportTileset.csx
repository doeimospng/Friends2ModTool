    ...
    foreach (Node n in atlas.Nodes)
    {
        if (n.Texture != null)
        {
            ...
            else if (spriteType == SpriteType.Tileset)
            {
                ...
                string getPaddingX = SimpleTextInput("Output Border X", "Enter value for x padding in the box below", "", false);
                if (String.IsNullOrEmpty(getPaddingX) || String.IsNullOrWhiteSpace(getPaddingX))
                {
                    ScriptError("Cannot be empty or null.");
                    return;
                }
                string getPaddingY = SimpleTextInput("Output Border Y", "Enter value for y padding in the box below", "", false);
                if (String.IsNullOrEmpty(getPaddingY) || String.IsNullOrWhiteSpace(getPaddingY))
                {
                    ScriptError("Cannot be empty or null.");
                    return;
                }
                string getTileWidth = SimpleTextInput("Tile Width", "Enter value for tile width in the box below", "", false);
                if (String.IsNullOrEmpty(getTileWidth) || String.IsNullOrWhiteSpace(getTileWidth))
                {
                    ScriptError("Cannot be empty or null.");
                    return;
                }
                string getTileHeight = SimpleTextInput("Tile Height", "Enter value for tile height in the box below", "", false);
                if (String.IsNullOrEmpty(getTileHeight) || String.IsNullOrWhiteSpace(getTileHeight))
                {
                    ScriptError("Cannot be empty or null.");
                    return;
                }
                string getTileColumns = SimpleTextInput("Number of Columns", "Enter number of columns in the box below", "", false);
                if (String.IsNullOrEmpty(getTileColumns) || String.IsNullOrWhiteSpace(getTileColumns))
                {
                    ScriptError("Cannot be empty or null.");
                    return;
                }
                string getTileCount = SimpleTextInput("Number of Tiles", "Enter (approximate) number of tiles in the box below", "", false);
